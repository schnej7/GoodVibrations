...
public class SetVolumeFunction extends Function
{
  ...
  public SetVolumeFunction execute()
  {
    ...
    // If the volume will be all the way down, and the vibrate button was
    // checked, then set the mode to vibrate
    ...
    byte flags = AudioManager.FLAG_SHOW_UI
    ...
    if ((volumeTypes & (byte) 1) != 0)
    {
      ...
      AM.setStreamVolume(AudioManager.STREAM_RING,
          scaleVolume(AudioManager.STREAM_RING), flags);
    }
    ...
    if ((volumeTypes & (byte) 2) != 0)
    {
      ...
      AM.setStreamVolume(AudioManager.STREAM_MUSIC,
          scaleVolume(AudioManager.STREAM_MUSIC), flags);
    }
    ...
    if ((volumeTypes & (byte) 4) != 0)
    {
      ...
      AM.setStreamVolume(AudioManager.STREAM_ALARM,
          scaleVolume(AudioManager.STREAM_ALARM), flags);
    }
    ...
    if ((volumeTypes & (byte) 8) != 0)
    {
      ...
      AM.setStreamVolume(AudioManager.STREAM_NOTIFICATION,
          scaleVolume(AudioManager.STREAM_NOTIFICATION), flags);
    }
  ...
  private SetVolumeFunction getInverse()
  {
    ...
    int currentVolume = Math.round((float) (AM
        .getStreamVolume(AudioManager.STREAM_RING) * 100.0) / maxVol);
  ...
  // scaleVolume
  // Takes the 0-100 value from the slider and rounds is to the nearest volume
  // setting for the proper stream
